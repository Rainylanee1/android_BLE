...
public class BluetoothLeDevice implements Parcelable {
    ...
    public boolean flag_TESS5600=false;
     ...
     */
    //每次搜索到一个蓝牙设备，自动添加item时会new 一个BluetoothLeDevice;
    public BluetoothLeDevice(final BluetoothDevice device, final int rssi, final byte[] scanRecord, final long timestamp) {
        ...
        if (mDevice.toString().equals("D2:88:25:6F:C8:38")) {
            flag_TESS5600=true;
        } //实现自动连接TESS5600蓝牙设备;

    }
    public boolean get_falg_TESS5600()
    {
        return flag_TESS5600;
    }
    /**
     * Instantiates a new Bluetooth LE device.
     *
